...
Public Class MainWindow
    ...
    Private Sub MoveRightButton(sender As Object, e As RoutedEventArgs)
        For i As Integer = Listbox1.SelectedItems.Count - 1 To 0 Step -1
            Listbox2.Items.Add(Listbox1.SelectedItem.Name.ToString)
        Next
    End Sub

    Private Sub MoveAllRightButton(sender As Object, e As RoutedEventArgs)
        Dim myItem As Object
        For Each myItem In Listbox1.Items
            Listbox2.Items.Add(myItem.Name.ToString)
        Next
    End Sub

    Private Sub MoveLeftButton(sender As Object, e As RoutedEventArgs)
        For i As Integer = Listbox2.SelectedItems.Count - 1 To 0 Step -1
            Listbox2.Items.Remove(Listbox2.SelectedItem)
        Next
    End Sub

    Private Sub MoveAllLeftButton(sender As Object, e As RoutedEventArgs)
        Listbox2.Items.Clear()
    End Sub

    Private Sub ShuffleUpButton(sender As Object, e As RoutedEventArgs)
        If Listbox2.Items.Count > 0 Then
            Dim index As Integer = Listbox2.SelectedIndex - 1
            If index >= 0 Then
    ...
    Private Sub ShuffleDownButton(sender As Object, e As RoutedEventArgs)
        If Listbox2.Items.Count > 0 Then
            Dim index As Integer = Listbox2.SelectedIndex + 1
            If index <= Listbox2.Items.Count - 1 Then
                Listbox2.Items.Insert(index + 1, Listbox2.SelectedItem)
                Listbox2.Items.RemoveAt(Listbox2.SelectedIndex)
                Listbox2.SelectedIndex = index
            End If
        End If
    ...
    Private Sub Listbox2_MouseDoubleClick(sender As Object, e As MouseButtonEventArgs) Handles Listbox2.MouseDoubleClick
        For i As Integer = Listbox2.SelectedItems.Count - 1 To 0 Step -1
            Listbox2.Items.Remove(Listbox2.SelectedItem)
        Next
    End Sub
...
#End Region
    ...
    Private Overloads Sub ApplyTemplate(sender As Object, e As RoutedEventArgs)

        'For i As Integer = 0 To pdSentTime.Items.Count - 1
        '    Dim s As String = pdSentTime.Items(i).Name.ToString()
        '    Dim first As String = pdSentTime.Items(0).Name.ToString()
        '    MsgBox(first)
        'Next


        FindSubStringIndex()
        globalBegDoc.SelectedIndex = 0
        globalBegAttach.SelectedIndex = 3



        tbText.Text = "The Parent/Date Hierarchy and Email Domain options are selected by applying LW Default Template. Selected fields may be customized and extra fields are avaialable to utilize."
        tbText.FontSize = "12"
        'pdSentDate.Text = "Select Date Sent"
        'pdSentTime.Text = "Select Time Sent"

        pdReceivedDate.Text = "Select Received Date"
        pdReceivedTime.Text = "Select Received Time"
        pdLastModDate.Text = "Select Last Mod Date"
        pdLastModTime.Text = "Select Last Mod Time"
        pdCreateDate.Text = "Select Create Date"
        pdCreateTime.Text = "Select Create Time"
        pdAccessDate.Text = "Select Access Date"
        pdAccessTime.Text = "Select Access Time"

        pdAddMore1.Text = "Extra Date Field"
        pdAddMore1b.Text = "Extra Date Field"
        pdAddMore1c.Text = "Extra Date Field"
        pdAddMore1d.Text = "Extra Date Field"
        pdAddMore1e.Text = "Extra Date Field"
        pdAddMore2.Text = "Extra Time Field"
        pdAddMore2b.Text = "Extra Time Field"
        pdAddMore2c.Text = "Extra Time Field"
        pdAddMore2d.Text = "Extra Time Field"
        pdAddMore2e.Text = "Extra Time Field"


        pdExpander.Visibility = Visibility.Visible
        edExpander.Visibility = Visibility.Visible
        ftExpander.Visibility = Visibility.Visible
        checkBox1.IsChecked = True
        checkBox3.IsChecked = True
        checkBox4.IsChecked = True
        pdExpander.IsExpanded = True
        edExpander.IsExpanded = True
        ftExpander.IsExpanded = True

        rb_internaldata.IsChecked = True
        edFrom.Text = "Select Email From"
        edTo.Text = "Select Email To"
        edCC.Text = "Select Email CC"
        edBCC.Text = "Select Email BCC"
        edSubject.Text = "Select Email Subject"



    End Sub

    Public Function FindSubStringIndex() As Integer

        Dim myItem As Object = New ObservableCollection(Of Object)
        Dim myItem2 As Object = New ObservableCollection(Of Object)

        Dim dateSearch As String = "date"

        For Each myItem In pdSentDate.Items
            Dim values As Object = myItem.Name.ToString

            If (values.Contains(dateSearch.ToUpper)) Then
                pdSentDate.Items.ToString.Reverse()
                pdSentDate.SelectedIndex = 0
                pdSentDate.Text = values
            End If
        Next

        Dim timeSearch As String = "time"

        For Each myItem2 In pdSentTime.Items
            Dim values As Object = myItem2.Name.ToString

            If (values.Contains(timeSearch.ToUpper)) Then
                pdSentTime.SelectedIndex = 0
                pdSentTime.Text = values
            End If
        Next
    ...
    Private Function ConvertTime(ByVal time As String) As String
        Dim reformattedTime24hour As String
        Dim index As Integer = time.IndexOf(":"c)

        If index < 0 Then
            Return 0
        Else
            Dim hours As String = time.Substring(0, index)


            Dim remainder As String = time.Substring(index)

            If remainder.LastIndexOf(":") = 0 Then
                remainder += ":00"
            End If

            Dim convertedHours As Integer = Int32.Parse(hours)

            If time.Contains("AM") Then
                If convertedHours = 12 Then convertedHours = 0
                remainder = remainder.Replace("AM", "")
            End If

            If time.Contains("PM") Then
                convertedHours += 12
                remainder = remainder.Replace("PM", "")
            End If

            If remainder.LastIndexOf(":"c) = (remainder.Length - 1) Then remainder = remainder.Substring(0, remainder.Length - 1)
            Dim outputHours As String = String.Format("{0:0}", convertedHours)
            outputHours = outputHours.PadLeft(2, "0"c)
            Dim result As String = outputHours & remainder
            reformattedTime24hour = result

            Return reformattedTime24hour
        End If
    End Function
    ...
    Private Sub Convert()
        Dim timeSearch As String = "time"
        Dim myItem As Object = New ObservableCollection(Of Object)
        For Each myItem In pdSentTime.Items
            Dim times As Object = myItem.Name.ToString
            ...
            For Each file In LoadFilesItemList
                ...
                Using ofl As FileStream = New FileStream(oinputfile, FileMode.Open, FileAccess.Read)
                    ...
                    End Using
                End Using
                If (times.Contains(timeSearch.ToUpper)) Then
                    For Each r In records
                        For Each dt In r.DateTimeItems
                            selectedTimeLabel.Content = pdSentTime.SelectedItem.Name.ToString
                            selectedTimeValue.Text = dt.FieldTime

                            convertedTime.Text = ConvertTime(dt.FieldTime)
                        Next
                    Next
                End If
            Next
